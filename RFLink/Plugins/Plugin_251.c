 ...
 * Author  (original) : esp8266nu 1 Okt 2016 RFLink 1.0
 * Support (original) : www.esp8266.nu
...
boolean PluginTX_251(byte function, char *string)
{
  ...
  long Par1 = 0;
  long Par2 = 0;
  if (Plugin_250_GetArgv(string, TmpStr1, 3))
    Par1 = atol(TmpStr1);
  if (Plugin_250_GetArgv(string, TmpStr1, 4))
    Par2 = atol(TmpStr1);
  ...
  if (strncasecmp_P(InputBuffer_Serial + 3, PSTR("COUNT"), 5) == 0)
  {
    ...
    if (PulseTimer[Par1 - 1] > 999999)
      PulseTimer[Par1 - 1] = 999999; // WebApp cannot handle larger values! (bug or by design?)
