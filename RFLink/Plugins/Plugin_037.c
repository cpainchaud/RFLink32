...
//#######################################################################################################
/*********************************************************************************************\
 * This plugin takes care of decoding the protocol used for outdoor sensors of the Accurite weather stations 
 ...
 *   1110 0100 0100 0111 1001 0100 0000 0001 0111 1100 00
 *   0110 0100 0100 0111 1001 0100 0000 0000 0100 1010 00
 *   0100 0000 0001 0100 1111 1001 0000 0001 0101 1110 00
 *
 ...
 *  \*********************************************************************************************/
#define ACCURITE_PLUGIN_ID 037
#define PLUGIN_DESC_037 "Accurite"
#define ACCURITE_PULSECOUNT 84

#define ACCURITE_MIDHI 2000 / RAWSIGNAL_SAMPLE_RATE
#define ACCURITE_PULSEMIN 150 / RAWSIGNAL_SAMPLE_RATE
#define ACCURITE_PULSEMINMAX 2500 / RAWSIGNAL_SAMPLE_RATE
#define ACCURITE_PULSEMAXMIN 650 / RAWSIGNAL_SAMPLE_RATE
...
{
   if (RawSignal.Number < ACCURITE_PULSECOUNT || RawSignal.Number > (ACCURITE_PULSECOUNT + 4))
      return false;
   ...
   {
      if (RawSignal.Pulses[x + 1] > ACCURITE_MIDHI)
         return false; // in between pulses should be short
      bitstream <<= 1;
      if (RawSignal.Pulses[x + 1] > ACCURITE_PULSEMAXMIN)
      {
         bitstream |= 0x1;
         Serial.print("1");
      }
   ...
   temperature = ((temperature - 32) * 5 / 9 * 10);   // Accurite sensors are in Farenheit
